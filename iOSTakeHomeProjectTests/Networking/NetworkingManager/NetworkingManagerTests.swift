...
//
//  Created by Marcin Jędrzejak on 28/08/2022.
//
...
//        _ = try await NetworkingManager.shared.request(session: session, -> Old
    ...
    func test_with_unsuccessful_response_code_in_invalid_range_is_invalid() async {
        ...
        do {
            ...
            guard let networkingError = error as? NetworkingManager.NetworkingError else {
                XCTFail("Go to the wrong type of error, expecting NetworkingManager NetworkingError")
                return
            }
    ...
    func test_with_successfull_response_with_invalid_json_is_invalid() async {
